...
def face_vector_from_bgr(
    ...
    """
    Retourne un vecteur normalisé (grayscale 128x128 aplati) du plus grand visage détecté.
...
def bgr_from_file(file) -> Optional[np.ndarray]:
    """
    Convertit un UploadedFile/BytesIO (Streamlit) en image BGR (np.ndarray).
    - D'abord OpenCV.
    - En secours (facultatif) tente Pillow si disponible (sans l'exiger).
    """
    if file is None:
        return None
    try:
    ...
    # Secours : Pillow si présent (sans dépendance obligatoire)
    try:
        from PIL import Image  # import local, non obligé dans requirements
        import io
...
def draw_faces(bgr: np.ndarray, color_bgr: Tuple[int, int, int],
               scale_factor: float, min_neighbors: int) -> Optional[np.ndarray]:
    """Dessine des rectangles sur une copie de l'image. None si entrée invalide."""
...
def safe_show_bgr(bgr: Optional[np.ndarray], caption: str, use_container_width: bool = True):
    """
    Affiche une image BGR si valide. N'émet pas d'erreur si None/invalide.
    Évite les crashs (ex. lors du déploiement) → corrige l'erreur d'origine.
    """
    if isinstance(bgr, np.ndarray) and bgr.ndim == 3 and bgr.shape[2] == 3:
        try:
            ...
            # On n'affiche rien si conversion impossible, pour ne pas planter l'app
            st.caption(f"⚠️ Impossible d'afficher « {caption} » (image invalide).")
...
defaults = {
    ...
    "ui_enabled": True,        # ✅ activation/désactivation demandées
}
...
with st.sidebar:
    ...
    st.divider()
    st.header("UI control")
    st.session_state.ui_enabled = st.checkbox("Enable verification UI", value=st.session_state.ui_enabled)
    st.caption("Uncheck to temporarily disable camera widgets & actions.")
...
with left:
    ...
    if not st.session_state.ui_enabled:
        st.info("🔌 Verification UI is disabled. Enable it in the sidebar to proceed.")
    else:
        ...
        if st.session_state.ref_img is not None:
            safe_show_bgr(st.session_state.ref_img, caption="Reference")
        ...
        if st.session_state.allow_persist:
            ...
            if proof_cam is not None:
                proof_bgr = bgr_from_file(proof_cam)
                if isinstance(proof_bgr, np.ndarray):
